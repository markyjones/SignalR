...
namespace Microsoft.AspNet.SignalR.Tests
{
    public class PersistentConnectionFacts
    {
        public class ProcessRequest
        {
            ...
            [Fact]
            public void UncleanDisconnectFiresOldOnDisconnectedMethod()
            {
                // Arrange
                var req = new Mock<IRequest>();
                req.Setup(m => m.Url).Returns(new Uri("http://foo"));
                ...
                var connection = new Mock<PersistentConnection>() { CallBase = true };
                var oldOnDisconnectedCalled = false;
                var newOnDisconnectedCalled = false;
                connection.Protected().Setup("OnDisconnected", req.Object, "1").Callback(() =>
                {
                    oldOnDisconnectedCalled = true;
                });
                connection.Protected().Setup("OnDisconnected", req.Object, "1", false).Callback(() =>
                {
                    newOnDisconnectedCalled = true;
                });
                ...
                Assert.True(oldOnDisconnectedCalled, "OnDisconnected(IRequest, string) should be called.");
                Assert.True(newOnDisconnectedCalled, "OnDisconnected(IRequest, string, bool) should be called.");
            }
        }
